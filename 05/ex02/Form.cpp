...
/*   Updated: 2021/04/30 13:49:36 by sglass           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form( std::string const &name, int gradeSign, int gradeExec, std::string const &target) : _name(name), _gradeSign(gradeSign), _gradeExec(gradeExec), _target(target)
{
	_isSigned = false;
	if (_name.compare("\0") == 0)
		throw(std::invalid_argument("no name"));
	if (_target.compare("\0") == 0)
		throw(std::invalid_argument("no target"));
...
Form::Form( Form const &src ) :  _name(src._name), _gradeSign(src._gradeSign), _gradeExec(src._gradeExec), _target(src._target)
{
...
{
	if (executor.getGrade() <= _gradeExec && _isSigned)
		action();
	if (!_isSigned)
	{
